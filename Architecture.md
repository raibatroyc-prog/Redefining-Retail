...
and stock-movement history. **(Updated 2026-09-11 — see "Phase 6" below.)**
Supplier and purchase-order routes now also support the write operations
described in Phase 6; the read-only list/detail views described above are
still present and unchanged. The inventory route also contains the transient
read-only AgentAssistant (the agent itself remains read-only/advisory — see
"Deterministic intelligence and agent safety" below — only the plain UI forms
added in Phase 6 write to the database).
...
The authenticated read APIs are:
...
**Updated 2026-09-11 — the authenticated write APIs (see "Phase 6" below)
are:**

- `POST /api/inventory` (create product — owner/manager only)
- `POST /api/inventory/:productId/movements` (record a stock transaction —
  any org member)
- `POST /api/suppliers` (create supplier — owner/manager only)
- `PATCH /api/suppliers/:supplierId` (update supplier — owner/manager only)
- `POST /api/purchase-orders` (create draft PO — owner/manager only)
- `POST /api/purchase-orders/:purchaseOrderId/items` (add line item —
  owner/manager only, draft POs only)
- `PATCH /api/purchase-orders/:purchaseOrderId/status` (advance/cancel a
  PO — owner/manager only, subject to the state machine in Phase 6)

The inventory list supports `status`, `category`, `lowStock`, `expiring`,
`search`, `limit`, and `offset`. Suppliers support `search`, `limit`, and
`offset`; purchase orders support `limit` and `offset`. All controllers
(read and write) require authentication, validate identifiers, authorize
organization membership, and verify resource ownership; write controllers
additionally enforce role-based access control (RBAC) via
`requireOrganizationRole()` in `backend/api/auth.ts`.
...
## Explicit Phase 5 boundary (superseded — see Phase 6)

As originally written, Phase 5 frontend routes did not expose product,
inventory, stock, supplier, or purchase-order creation, editing, deletion,
approval, cancellation, receiving, stock adjustment, supplier contact,
email, or messaging workflows, even though backend service functions for
several of these already existed unconnected. **This boundary was removed
on 2026-09-11 for the operations listed in Phase 6 below.** Supplier
contact/email/messaging workflows remain out of scope.

## Phase 6 — Write operations added (2026-09-11)

To satisfy the CIA-III "meaningful business operations", "management
operations", and "transaction/status information" requirements, the
following write paths were connected end-to-end (UI → API → service →
database) using the backend service functions that already existed
(`createProduct`, `addStockMovement`, `createSupplier`, `updateSupplier`,
`createPurchaseOrder`, `addPurchaseOrderItem`, `updatePurchaseOrderStatus`
in `backend/services/*`), which were previously unreachable from any route:

- **User-side (any org member — staff/manager/owner):** record a stock
  transaction (`receipt` / `sale` / `waste` / `adjustment`) against a
  product, via a form on `/inventory/$productId`
  (`frontend/src/components/stock-movement-form.tsx` →
  `POST /api/inventory/:productId/movements` →
  `backend/api/controllers/inventory.controller.ts#recordMovement` →
  `inventory.service.ts#addStockMovement`). A business rule rejects a
  `sale`/`waste` movement that would exceed current stock on hand.
- **Manager/Admin side (owner/manager only, enforced server-side via
  `requireOrganizationRole()`):**
  - Add a new product/SKU (`/inventory` →
    `product-create-form.tsx` → `POST /api/inventory`).
  - Add a new supplier (`/suppliers` → `supplier-form.tsx` →
    `POST /api/suppliers`) and update an existing supplier's
    delivery-performance status (`/suppliers/$supplierId` →
    `supplier-status-form.tsx` → `PATCH /api/suppliers/:id`).
  - Create a draft purchase order against a supplier
    (`/purchase-orders` → `purchase-order-create-form.tsx` →
    `POST /api/purchase-orders`), add line items to a draft PO
    (`/purchase-orders/$purchaseOrderId` →
    `purchase-order-item-form.tsx` →
    `POST /api/purchase-orders/:id/items`), and advance/cancel a PO
    (`purchase-order-status-actions.tsx` →
    `PATCH /api/purchase-orders/:id/status`), enforced by an explicit
    `draft → sent → received | cancelled` state machine
    (`PO_STATUS_TRANSITIONS` in `purchase-order.controller.ts`) that
    rejects illegal transitions (e.g. skipping straight to `received`,
    or re-sending a `cancelled` order).

RBAC is enforced by `requireOrganizationRole()` in `backend/api/auth.ts`,
which looks up the caller's `org_members.role` for the target organization
and returns `403 Forbidden` if it is not in the allowed set for that
operation. All new endpoints reuse the existing authentication
(`requireAuthentication`) and organization-membership
(`authorizeOrganizationAccess`) checks already used by the read endpoints.

Automated coverage for the new routing and error-mapping behaviour is in
`tests/write-operations.test.ts`. **This was verified by source inspection
and the TypeScript parser only; `npm run typecheck` / `npm test` could not
be executed in the environment these changes were made in (no network
access to install dependencies) — run them yourself before submission to
get a real pass/fail result.**
