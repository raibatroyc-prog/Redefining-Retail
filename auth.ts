...
export async function getOrganizationRole(
  ...
  if (error) {
    throw new ApiError("Failed to verify organization role", 500, { code: "role_lookup_failed" });
  }
...
/**
 * Enforces role-based access control (RBAC) for write/management operations.
 * `owner` and `manager` are treated as management roles; `staff` is the
 * front-line/user role. Throws 403 Forbidden if the caller's role in the
 * organization is not one of `allowedRoles`.
 */
export async function requireOrganizationRole({
  ...
  const role = await getOrganizationRole(userId, organizationId, supabase);

  if (!role) {
    throw new ApiError("Forbidden", 403, { code: "forbidden" });
  }

  if (!allowedRoles.includes(role as "owner" | "manager" | "staff")) {
    throw new ApiError(
      "You do not have permission to perform this action.",
      403,
      { code: "insufficient_role", requiredRoles: allowedRoles, actualRole: role },
    );
  }
